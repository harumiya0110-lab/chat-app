(() => {
  ...
  function ensurePopupActions(marker) {
    const popup = marker?.getPopup?.();
    const popupElement = popup?.getElement?.();
    const content = popupElement?.querySelector?.('.leaflet-popup-content');
    if (!content) return null;

    let actions = content.querySelector('.map-pin-actions');
    if (!actions) {
      actions = document.createElement('div');
      actions.className = 'map-pin-actions';
  ...
  function refreshPopupActions(marker) {
    ...
    const actions = ensurePopupActions(marker);
    if (!actions) return;
    actions.innerHTML = '';
    renderHelpStatus(actions, marker);
  ...
  window.ruralRefreshMapPopupActions = refreshPopupActions;
  ...
  function addDeleteControl(marker) {
    ...
    // client.js が住所確認などで setPopupContent() を実行しても、
    // 「行ってみたい」「手伝える」などの操作欄が消えないように復元します。
    popup.on?.('contentupdate', () => {
      window.setTimeout(() => {
        if (marker.isPopupOpen?.()) refreshPopupActions(marker);
      }, 0);
    });
  }

(() => {
  if (window.__ruralChatCustomizationUI) return;
  window.__ruralChatCustomizationUI = true;
  ...
  function decorateAvatars() {
    ...
    document.querySelectorAll('.message').forEach(item => {
      ...
      if (!name || name.classList.contains('chat-avatar')) return;
      const username = name.textContent.replace(/\s+あなた$/, '').trim();
      const avatar = document.createElement('span');
      avatar.className = 'chat-avatar';
      avatar.textContent = avatarInitial(username);
      avatar.dataset.frame = item.classList.contains('own') ? frame : 'default';
    ...
    document.querySelectorAll('#users-list li').forEach(li => {
      ...
      if (!name || li.querySelector('.chat-avatar')) return;
      const avatar = document.createElement('span');
      avatar.className = 'chat-avatar';
      avatar.textContent = avatarInitial(name.textContent);
      avatar.dataset.frame = name.textContent.trim() === currentUsername ? frame : 'default';
  ...
  bgGrid.addEventListener('click', event => {
    ...
    if (customState.backgrounds.includes(id)) exchange('select-chat-background', id, button);
    else exchange('exchange-chat-background', id, button);
  });
  frameGrid.addEventListener('click', event => {
    ...
    if (customState.iconFrames.includes(id)) exchange('select-chat-icon-frame', id, button);
    else exchange('exchange-chat-icon-frame', id, button);
  });

  const observer = new MutationObserver(() => decorateAvatars());
  observer.observe(document.getElementById('messages') || document.body, { childList: true, subtree: true });
  observer.observe(document.getElementById('users-list') || document.body, { childList: true, subtree: true });
  window.setInterval(decorateAvatars, 1500);
  render();
})();

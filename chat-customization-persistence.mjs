...
export const CHAT_ICON_FRAME_CATALOG = {
...
function cloneState(state = DEFAULT_STATE) {
  return {
    ...
    iconFrames: [...new Set(['default', ...(Array.isArray(state.iconFrames) ? state.iconFrames : [])])].filter(id => CHAT_ICON_FRAME_CATALOG[id]),
    currentIconFrame: CHAT_ICON_FRAME_CATALOG[state.currentIconFrame] ? state.currentIconFrame : 'default'
  };
}
...
async function selectCustomization(socket, kind, id, ack) {
  const username = String(socket.__chatCustomizationUsername || '').trim();
  const catalog = kind === 'background' ? CHAT_BACKGROUND_CATALOG : CHAT_ICON_FRAME_CATALOG;
  if (!username || !catalog[id]) return fail(ack, 'invalid');
  return withLock(username, async () => {
...
export function registerChatCustomizationPersistence(io) {
  if (!io || io.__chatCustomizationRegistered) return;
  io.__chatCustomizationRegistered = true;
